...
function init (client, $) {
  ...
  careportal.filterInputs = function filterInputs (event) {
    ...
    // Daily Bolus and Daily Basal default to "other" time at 00:00 today
    if (eventType === 'Daily Bolus' || eventType === 'Daily Basal') {
      $('#othertime').prop('checked', true);
      setDateAndTime(client.ctx.moment().startOf('day'));
    }
  ...
  careportal.prepare = function prepare () {
    ...
    $('#preBolus').val(0);
    $('#notes').val('');
    $('#enteredBy').val((client.authorized && client.authorized.sub) ? client.authorized.sub : storage.get('enteredBy') || '');
  ...
  function gatherData () {
    ...
    // For Daily Bolus and Daily Basal, record a UTC timestamp note and leave enteredBy blank
    if (data.eventType === 'Daily Bolus' || data.eventType === 'Daily Basal') {
      data.notes = JSON.stringify({ updatedAt: new Date().toISOString(), updatedBy: 'careportal' });
      data.enteredBy = '';
    }
