...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  function drawChart (day, data, options) {
    ...
    data.treatments.forEach(function(treatment) {
      if (treatment.eventType === 'Daily Basal' || treatment.eventType === 'Daily Bolus') {
        return;
      }
    ...
    function dailyInsulinTotal (eventType) {
      var total;
      data.treatments.forEach(function(treatment) {
        if (treatment.eventType !== eventType) {
          return;
        }

        if (treatment.insulin === undefined || treatment.insulin === null || treatment.insulin === '') {
          return;
        }

        var insulin = Number(treatment.insulin);
        if (!isNaN(insulin)) {
          total = insulin;
        }
      });
    ...
    bolusInsulin = dailyInsulinTotal('Daily Bolus');
    var totalBasalInsulin = dailyInsulinTotal('Daily Basal');
      ...
      var chartData = [
        {
          ...
          , pct: (totalDailyInsulin > 0 ? totalBasalInsulin / totalDailyInsulin * 100 : 0).toFixed(0)
        }
        , { label: translate('Bolus'), count: bolusInsulin, pct: (totalDailyInsulin > 0 ? bolusInsulin / totalDailyInsulin * 100 : 0).toFixed(0) }
      ];
