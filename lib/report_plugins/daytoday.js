...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  function drawChart (day, data, options) {
    ...
    data.treatments.forEach(function(treatment) {
      if (treatment.eventType === 'Daily Basal' || treatment.eventType === 'Daily Total') {
        return;
      }
    ...
    if (options.insulindistribution) {
      ...
      var glookoDailyTotals = data.treatments.filter(function(t) {
        return t.eventType === 'Daily Basal' || t.eventType === 'Daily Total';
      });
      if (glookoDailyTotals.length > 0) {
        var glookoTotal = glookoDailyTotals[glookoDailyTotals.length - 1];
        var basalAmount = (glookoTotal.basal !== undefined && glookoTotal.basal !== null) ? glookoTotal.basal : glookoTotal.insulin;
        if (basalAmount !== undefined && basalAmount !== null) {
          totalBasalInsulin = Number(basalAmount);
        }
      }

      $('<tr><td><b>' + translate('Total basal insulin:') + '</b></td><td align="right">' + totalBasalInsulin.toFixed(1) + 'U</td></tr>').appendTo(table);
      var totalDailyInsulin = bolusInsulin + totalBasalInsulin; // Ensure TDD is updated based on the overridden basal
