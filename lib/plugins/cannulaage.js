...
function init(ctx) {
  ...
  cage.findLatestTimeChange = function findLatestTimeChange (sbx) {
    ...
    if (cannulaInfo.found) {
      ...
      cannulaInfo.displayLong = '';
      if (cannulaInfo.age >= 24) {
        cannulaInfo.displayLong += cannulaInfo.days + ' ' + translate('days');
      }
      if (cannulaInfo.displayLong.length > 0) {
        cannulaInfo.displayLong += ' ';
      }
      cannulaInfo.displayLong += cannulaInfo.hours + ' ' + translate('hours');
    } else {
      cannulaInfo.display = 'n/a ';
      cannulaInfo.displayLong = 'n/a ';
    }
  ...
  cage.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if (cannulaInfo.found) {
      info.push({ label: translate('Inserted'), value: new Date(cannulaInfo.treatmentDate).toLocaleString() });
      info.push({ label: translate('Duration'), value: cannulaInfo.displayLong });
