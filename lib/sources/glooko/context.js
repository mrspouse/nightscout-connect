...
function createNightscoutHelper(config) {
   ...
   */
  async function fetchExistingDailyTotals(jwt) {
    var res = await axios(
      baseUrl + '/api/v3/treatments?eventType=Daily%20Total&sort$desc=created_at&limit=200',
      { headers: { 'Authorization': 'Bearer ' + jwt }, timeout: REQUEST_TIMEOUT }
    );
    var data = res.data;
    return (Array.isArray(data) ? data : (data?.result || [data])).filter(Boolean);
   ...
   * @returns {Promise<{ existingSiteChanges: Array, existingAlarms: Array, existingDailyTotals: Array, fetchBaselineTotal: Function }>}
   */
  async function buildContext(options) {
    ...
    if (!needsSiteChanges && !needsAlarms && !needsDailyTotals) {
      return context;
    }
    ...
    try {
      ...
      if (needsDailyTotals) {
        context.existingDailyTotals = await fetchExistingDailyTotals(jwt);
      }
    } catch (e) {
  ...
  async function fetchBaselineTotal(lastSiteChangeTreatment) {
    ...
    try {
      ...
      var baselineRes = await axios(
        baseUrl + '/api/v3/devicestatus?lastSiteChange=' + siteChange + '&sort$asc=created_at&limit=1',
        { headers: { 'Authorization': 'Bearer ' + jwt }, timeout: REQUEST_TIMEOUT }
      );
