...
function array_from_endpoint(results, endpointName, nestedKey) {
  ...
  if (data && Array.isArray(data[nestedKey])) {
    console.log('nestedKey', nestedKey, data[nestedKey]);
    console.log("--");
...
function objects_from_reservoir_change(results, endpointName) {
  var endpoint = results[endpointName] || {};
  console.log(endpointName, endpoint);
  var data = endpoint.data || {};
  console.log(endpoint.data);
  var resChange = data.series && data.series.reservoirChange;
    console.log('resChange',resChange);
...
function objects_from_daily_totals(results, endpointName) {
  ...
  var data = endpoint.data || {};
  console.log(endpoint.data);
  var dayTotals = data.series && data.series.dailyInsulinTotals;
...
function with_integration_summary(batch) {
  ...
  try {
    ...
    return Object.assign({}, inputBatch, {
      foods: array_from_endpoint(results, 'Foods', 'foods'),
      insulins:
        array_from_endpoint(results, 'Insulins', 'insulins'),
      normalBoluses:
