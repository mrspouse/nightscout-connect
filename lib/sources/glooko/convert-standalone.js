...
function generate_nightscout_treatments(batch, timestampDelta) {
  ...
  console.log("DAY TOTAL DELTAS  ", totalPumpInsulinPerDayDeltas );
  ...
  if (totalPumpInsulinPerDayDeltas.length) {
    totalPumpInsulinPerDayDeltas.forEach(function (element) {
      if (!Number.isFinite(element.pumpInsulinDelta)) {
        return;
      }

      var treatment = {
        eventType: 'Device Status',
        eventTime: new Date(new Date(element.timestamp).getTime() + timestampDelta).toISOString(),
        notes:
          'Glooko totalPumpInsulinPerDay delta: ' +
          element.pumpInsulinDelta +
          'U (current: ' +
          element.totalPumpInsulinPerDay +
          'U, previous day: ' +
          Number((element.totalPumpInsulinPerDay - element.pumpInsulinDelta).toFixed(2)) +
          'U)',
      };
