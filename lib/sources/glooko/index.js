 ...
 * and others.
 * 
 * Enhanced with Puppeteer-based browser authentication to enable Glooko to re-sync with Omnipod 5
...
const { glookoConnect } = require('./glooko-login.js'); // Puppeteer browser login and data retrieval
...
function glookoSource(opts) {
  ...
  var impl = {
    ...
    async dataFromSesssion() {
      // run Puppeteer login each time data is polled to enable Omnipod 5 to re-sync
    ...
    transformData(batch) {
      ...
      const { treatments, devicestatus } = helper.generate_nightscout_treatments(
        batch,
  ...
  function tracker_for() {
    var AxiosTracer = require("../../trace-axios.js");
  ...
  function generate_driver(builder) {
    ...
    builder.register_loop("Glooko", {
      ...
      // expect new data 20 minutes after last success
      // originally 5 but increase to avoid overload as new login is called each time
      expected_data_interval_ms: 20 * 60 * 1000,
      backoff: {
        // wait 2.5 minutes * 2^attempt
        interval_ms: 2.5 * 60 * 1000,
      },
