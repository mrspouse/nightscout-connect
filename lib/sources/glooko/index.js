...
var Defaults = {
  ...
  // Enhanced endpoint definitions from glooko-login.js
  endpoints: [
...
/**
 * Puppeteer-based Glooko login with automatic patient ID extraction
 * @param {object} opts - Configuration options
 * @param {string} opts.glookoEmail - Glooko account email
 * @param {string} opts.glookoPassword - Glooko account password
 * @param {string} [opts.glookoEnv] - Glooko environment (default, development, production, eu)
 * @param {string} [opts.glookoServer] - Custom Glooko server URL
 * @param {import('axios').AxiosStatic} axios - Axios instance
 * @returns {Promise<{cookies: string, user: any, patientId: string}>}
 */
async function glookoPuppeteerLogin(opts, axios) {
  let browser;
  
  try {
    console.log('🔐 GLOOKO: Browser authentication starting...');
    
    // Determine web and API URLs based on environment
    const webUrl = opts.glookoEnv === 'production' 
      ? 'https://my.glooko.com'
      : opts.glookoEnv === 'development'
      ? 'https://development.glooko.com'
      : 'https://eu.my.glooko.com'; // default to EU
    
    const apiUrl = opts.glookoServer 
      ? `https://${opts.glookoServer}`
      : _known_servers[opts.glookoEnv || 'default']
      ? `https://${_known_servers[opts.glookoEnv || 'default']}`
      : 'https://eu.api.glooko.com';

    console.log(`📋 Using URLs - Web: ${webUrl}, API: ${apiUrl}`);
    ...
    // Extract patient ID from DOM
    console.log('👤 Extracting patient ID...');
    ...
    const patientId = await page.evaluate(() => {
      return window.patient || 
             ...
             window.patientId ||
             document.body.dataset.patient ||
             document.body.dataset.glookoCode;
    });
    ...
    // Return authentication result compatible with session system
    const result = {
    ...
    console.error('❌ GLOOKO: Authentication failed -', error.message);
...
function glookoSource(opts, axios) {
  ...
  var impl = {
    authFromCredentials() {
      // Use Puppeteer-based browser authentication for more reliable login
      return glookoPuppeteerLogin(opts, axios);
    },

    /**
     * @param {{cookies: string, user: any}} auth
     * @returns {Promise<{cookies: string, user: any, endpoints: Array}>}
     */
    async sessionFromAuth(auth) {
      console.log('📋 Initializing Glooko session with endpoints...');
      
      // Create axios instance with authentication headers from cookies
      ...
      // Test endpoints and build endpoint configuration
      const testedEndpoints = [];
      for (const endpoint of Defaults.endpoints) {
        try {
          ...
          console.log(`⚠️  ${endpoint.name} test skipped: ${error.message}`);
          testedEndpoints.push({
            ...endpoint,
            tested: false,
            status: 'error',
      ...
      // Return enhanced session with endpoint configuration
