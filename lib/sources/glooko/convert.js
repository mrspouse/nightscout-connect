...
async function generate_nightscout_treatments(batch, timestampDelta, nsContext) {
  ...
  if (totalInsulinPerDay) {
    totalInsulinPerDay
      ...
      // Use UTC comparison for daily markers from Glooko.
      // If the marker falls on the same day as our sync, anchor it to sync time
      // so it isn't hidden in the future by Nightscout.
      if (moment.utc(baseTimestamp).isSame(moment.utc(lastSync), 'day')) {
        dailyTotalTreatment.created_at = lastSyncISO;
        createdAt = lastSyncISO;
      }

      var existing = (nsContext.existingDailyTotals || []).find(function (t) {
