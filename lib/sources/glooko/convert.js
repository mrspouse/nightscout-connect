...
async function generate_nightscout_treatments(batch, timestampDelta, nsContext) {
  ...
  //   existingDailyTotals: Array — pre-fetched Daily Total treatments
  //   fetchBaselineTotal: async Function — fetches insulin baseline (called mid-transform)
  nsContext = nsContext || { existingSiteChanges: [], existingAlarms: [], existingDailyTotals: [] };
  ...
  var existingDailyTotals = nsContext.existingDailyTotals || [];
  ...
  existingDailyTotals.forEach(function(t) {
    var s = t.syncTimestamp || t.lastSync || t.lastSyncISO;
    if (s && allKnownSyncs.indexOf(s) === -1) {
      allKnownSyncs.push(s);
    }
  ...
  if (totalInsulinPerDay) {
    totalInsulinPerDay.forEach(function(element) {
      if (element.basalUnitsPerDay !== undefined && element.basalUnitsPerDay !== null) {
        var baseTimestamp =
          element.timestamp ||
          (Number.isFinite(element.mills)
            ? new Date(element.mills).toISOString()
        ...
        var dailyTotalTreatment = {};
        dailyTotalTreatment.eventType = 'Daily Total';
        var createdAt = new Date(f_time + timestampDelta).toISOString();
        dailyTotalTreatment.created_at = createdAt;
        dailyTotalTreatment.basal = Number(element.basalUnitsPerDay);
        dailyTotalTreatment.totalInsulin = insulinCalc.insulin_total_value(element);
        ...
        var existing = existingDailyTotals.find(function(t) {
          // Primary: exact created_at match (works when timestampDelta is stable).
          if (moment(t.created_at).valueOf() === moment(createdAt).valueOf()) {
            return true;
          }
        ...
        if (existingSync) {
          dailyTotalTreatment.syncTimestamp = existingSync;
        } else if (lastSyncISO) {
          dailyTotalTreatment.syncTimestamp = lastSyncISO;
        }

        if (dailyTotalTreatment.syncTimestamp) {
          var ordered = { pumpSync: dailyTotalTreatment.syncTimestamp };
          Object.assign(ordered, element);
          delete ordered.timestamp;
          delete ordered.x;
          delete ordered.y;
        ...
        dailyTotalTreatment.notes = JSON.stringify(element);

        treatments.push(dailyTotalTreatment);
      }
    });
    console.log('totalInsulinPerDay elements processed as Daily Totals:', totalInsulinPerDay.length);
  }
  ...
  if (totalInsulinPerDay && lastSync) {
    ...
    if (totalInsulinPerDay) {
      deviceStatus.InsulinPerDay = insulinCalc.format_insulin_per_day(totalInsulinPerDay, lastSiteChangeTreatment, lastSync);
