...
async function generate_nightscout_treatments(batch, timestampDelta, nsContext) {
  ...
  if (totalInsulinPerDay) {
    totalInsulinPerDay
      ...
      var f_time = new Date(baseTimestamp).getTime();
      var createdAt = moment.utc(f_time).startOf('day').add(timestampDelta, 'ms').toISOString();

      var dailyTotalBasal = {};
      dailyTotalBasal.eventType = 'Daily Basal';
      dailyTotalBasal.created_at = createdAt;

      var existing = (nsContext.existingDailyBasal || []).find(function (t) {
        return moment.utc(t.created_at).isSame(moment.utc(createdAt), 'day');
      });

      var glookoDailyBasal = Number(element.basalUnitsPerDay);
      var existingDailyBasal = existing ? Number(existing.insulin) : NaN;
      if (Number.isFinite(glookoDailyBasal) && Number.isFinite(existingDailyBasal)) {
        dailyTotalBasal.insulin = Math.max(glookoDailyBasal, existingDailyBasal);
      } else if (Number.isFinite(glookoDailyBasal)) {
        dailyTotalBasal.insulin = glookoDailyBasal;
      } else if (Number.isFinite(existingDailyBasal)) {
        dailyTotalBasal.insulin = existingDailyBasal;
      }
      ...
      if (existingSync) {
        dailyTotalBasal.syncTimestamp = existingSync;
      } else if (lastSyncISO) {
        dailyTotalBasal.syncTimestamp = lastSyncISO;
      }

      if (dailyTotalBasal.syncTimestamp) {
        var ordered = { pumpSync: dailyTotalBasal.syncTimestamp };
        Object.assign(ordered, element);
        delete ordered.timestamp;
        delete ordered.x;
        delete ordered.y;
      ...
      dailyTotalBasal.notes = JSON.stringify(element);

      treatments.push(dailyTotalBasal);
    });
    console.log('totalInsulinPerDay treatments processed:', totalInsulinPerDay.length);
  }
